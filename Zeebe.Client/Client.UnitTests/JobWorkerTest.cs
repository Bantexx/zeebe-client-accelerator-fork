...
public class JobWorkerTest : BaseZeebeTest
{
    ...
    public void ShouldSendRequestReceiveResponseAsExpected()
    {
        ...
        Assert.That(receivedJobs.Count, Is.EqualTo(3));
    ...
    public void ShouldReceiveStreamedJobsWhenEnabled()
    {
        ...
        Assert.That(expectedStreamRequest, Is.EqualTo(actualRequest));

        Assert.That(3, Is.EqualTo(receivedJobs.Count));
        AssertJob(receivedJobs[0], 1);
        AssertJob(receivedJobs[1], 2);
        AssertJob(receivedJobs[2], 3);
    }
    ...
    public void ShouldFailWithZeroThreadCount()
    {
        ...
        Assert.That(aggregateException.Message, Does.Contain("Expected an handler thread count larger then zero, but got 0."));
    }
    ...
    public void ShouldSendAsyncCompleteInHandler()
    {
        ...
        Assert.That(expectedRequest, Is.EqualTo(actualActivateRequest));
    ...
    public void ShouldSendRequestsWithDifferentAmounts()
    {
        ...
        Assert.That(expectedSecondRequest, Is.EqualTo(actualSecondRequest));
    }
    ...
    public void ShouldSendFailCommandOnExceptionInJobHandler()
    {
        ...
        Assert.That(expectedFailRequest, Is.EqualTo(actualFailRequest));
    }
    ...
    public void ShouldCompleteAfterSendFailCommandOnExceptionInJobHandler()
    {
        ...
        Assert.That(1, Is.EqualTo(((CompleteJobRequest)completeRequests[0]).JobKey));
        Assert.That(3, Is.EqualTo(((CompleteJobRequest)completeRequests[1]).JobKey));
    }
    ...
    public void ShouldUseAutoCompleteWithWorker()
    {
        ...
        var completeJobRequests = TestService.Requests[typeof(CompleteJobRequest)].OfType<CompleteJobRequest>()
            .Select(j => j.JobKey).ToList();
        Assert.That(3, Is.EqualTo(completeJobRequests.Count));

        Assert.That(completeJobRequests, Contains.Item(1));
        Assert.That(completeJobRequests, Contains.Item(2));
        Assert.That(completeJobRequests, Contains.Item(3));
    }
    ...
    {
        Assert.That(expectedKey, Is.EqualTo(job.Key));
        Assert.That(3, Is.EqualTo(job.Retries));
        Assert.That("foo", Is.EqualTo(job.Type));
        Assert.That("jobWorker", Is.EqualTo(job.Worker));

        Assert.That("{\"foo\":" + expectedKey + "}", Is.EqualTo(job.Variables));
        Assert.That("{\"customFoo\":\"" + expectedKey + "\"}", Is.EqualTo(job.CustomHeaders));

        Assert.That("process", Is.EqualTo(job.BpmnProcessId));
        Assert.That("job" + expectedKey, Is.EqualTo(job.ElementId));
        Assert.That(23, Is.EqualTo(job.ElementInstanceKey));
        Assert.That(29, Is.EqualTo(job.ProcessInstanceKey));
        Assert.That(3, Is.EqualTo(job.ProcessDefinitionVersion));
        Assert.That(21, Is.EqualTo(job.ProcessDefinitionKey));
    }
}
