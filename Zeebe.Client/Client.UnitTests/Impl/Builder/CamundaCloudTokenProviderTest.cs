...
public class CamundaCloudTokenProviderTest
{
    ...
    private class HttpMessageHandlerStub : HttpMessageHandler
    {
        ...
        {
            CheckDisposed();
            Assert.That(request.RequestUri, Is.EqualTo(_requestUri));
            var content = await request.Content.ReadAsStringAsync(cancellationToken);
            var queryString = HttpUtility.ParseQueryString(content);
            Assert.That(queryString["client_id"], Is.EqualTo(_clientId));
            Assert.That(queryString["client_secret"], Is.EqualTo(_clientSecret));
            Assert.That(queryString["audience"], Is.EqualTo(_audience));
    ...
    public async Task ShouldRequestCredentials()
    {
        ...
        // then
        Assert.That("REQUESTED_TOKEN", Is.EqualTo(token));
        Assert.That(1, Is.EqualTo(MessageHandlerStub.RequestCount));
    }
    ...
    public async Task ShouldNotThrowObjectDisposedExceptionWhenTokenExpires()
    {
        ...
        // then
        Assert.That(2, Is.EqualTo(MessageHandlerStub.RequestCount));
    }
}