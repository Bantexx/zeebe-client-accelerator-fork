...
public class PersistedAccessTokenCacheTest
{
    ...
    public async Task ShouldCacheToken()
    {
        ...
        // then
        Assert.That("token-0", Is.EqualTo(token));
    ...
    public async Task ShouldCacheTokenForDifferentAudience()
    {
        ...
        // then
        Assert.That("token-0", Is.EqualTo(firstToken));
        Assert.That("token-1", Is.EqualTo(secondToken));
        Assert.That(2, Is.EqualTo(fetchCounter));
    }
    ...
    public async Task ShouldResolveNewTokenAfterExpiry()
    {
        ...
        // then
        Assert.That("token-1", Is.EqualTo(token));
        Assert.That(2, Is.EqualTo(fetchCounter));
    }
    ...
    public async Task ShouldPersistTokenToDisk()
    {
        ...
        Assert.That(1, Is.EqualTo(fileNames.Length));
