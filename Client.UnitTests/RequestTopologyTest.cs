...
public class RequestTopologyTest : BaseZeebeTest
{
    ...
    public async Task ShouldReceiveResponseAsExpected()
    {
        ...
        Assert.That("host0:26501", Is.EqualTo(firstBroker.Address));
        Assert.That(0, Is.EqualTo(firstBroker.NodeId));

        var firstPartition = firstBroker.Partitions[0];
        Assert.That(0, Is.EqualTo(firstPartition.PartitionId));
        Assert.That(PartitionBrokerRole.LEADER, Is.EqualTo(firstPartition.Role));

        var secondBroker = response.Brokers[1];
        Assert.That("host1:26501", Is.EqualTo(secondBroker.Address));
        Assert.That(1, Is.EqualTo(secondBroker.NodeId));

        firstPartition = secondBroker.Partitions[0];
        Assert.That(0, Is.EqualTo(firstPartition.PartitionId));
        Assert.That(PartitionBrokerRole.FOLLOWER, Is.EqualTo(firstPartition.Role));

        var thirdBroker = response.Brokers[2];
        Assert.That("host2:26501", Is.EqualTo(thirdBroker.Address));
        Assert.That(2, Is.EqualTo(thirdBroker.NodeId));
